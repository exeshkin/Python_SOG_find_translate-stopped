...
def creat_file_list_duplicate_files():
    files = os.listdir('b_duplicate_files')
    for file in files:
        list_files.append(file[:-4])


def temp_sermon_to_fix_sermon(file=str):
    with open(f'b_duplicate_files/{file}.txt', encoding='utf-8') as file_temp, open('c_/temp_fix.txt', 'w', encoding='utf-8') as file_temp_fix:
        sermon_one_str = ''
...
def temp_fix_to_file_excl(file=str):
    with open('c_/temp_fix.txt', encoding='utf-8') as file_temp_fix, open(f'c_/duplicate_fix/{file}_tr.excl', 'a', encoding='utf-8') as file_temp_finish:
        text = ''
...
def go():
    creat_file_list_duplicate_files()

    for file in list_files:
        temp_sermon_to_fix_sermon(file)
        temp_fix_to_file_excl(file)
